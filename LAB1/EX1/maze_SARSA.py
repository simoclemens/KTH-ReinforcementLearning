...
class Maze:
    ...
    def sarsa(self, start=(0,0), num_episodes=1000, alpha=2/3, gamma=0.99, epsilon=0.1):
        # Initialize Q-table
        ns = len(self.states)
        na = len(self.actions)
        Q = np.zeros((ns, na))
        counter = np.zeros((ns, na))
        ...
        for episode in range(num_episodes):
            ...
            while True:
                ...
                done = self.finished(next_state, death_prob=1/50)
                ...
                step = 1/(n**alpha)
                Q[state, action] = Q[state, action] + step * (
